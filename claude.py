...
def load_images(directory):
    """Liest alle Bilder ein, kodiert sie in Base64 und bereitet sie für Claude vor."""
    images = []
    counter = 1
    for filename in os.listdir(directory):
        file_path = os.path.join(directory, filename)
        if os.path.isfile(file_path) and filename.lower().endswith(IMAGE_EXTENSIONS):
            with open(file_path, "rb") as img_file:
                ...
                # Bild als Claude-konforme Struktur hinzufügen
                images.append(
                    {
                        "type": "text",
                        "text": f"Image {counter}:"
                    },
                )
                images.append(
                    {
...
def send_to_ai(filedict, images, accessibility_report):
    ...
    content = [
        {
            "type": "text",
            "text": "Mache die Website so WCAG konform wie nur möglich, das Ziel ist es AAA Konformität zu erreichen. Beachte den mitgeschickten Report und behebe außerdem die dort aufgezeigten Fehler."
        }
    ]
