            ...
            steps {
                script {
                    sh '''
                        ...
                        docker run -d --network=${NETWORK_NAME} -p 3002:3002 --name ${CONTAINER_NAME} ${IMAGE_NAME}:${IMAGE_TAG}
            ...
            steps {
                script {
                    sh '''
                        ...
                        docker build -t pa11y-ci-tester-claude -f Dockerfile.pa11y .

                        echo "Erstelle Reports-Ordner..."
                        mkdir -p ${REPORTS_DIR}

                        echo "Starte Accessibility-Tests (Erster Durchlauf)..."
                        docker run --rm --network=${NETWORK_NAME} -v $PWD/${REPORTS_DIR}:/app/reports pa11y-ci-tester-claude || true
            ...
            steps {
                script {
                    sh '''
                        ...
                        docker run -d --network=${NETWORK_NAME} -p 3002:3002 --name ${CONTAINER_NAME} ${TRANSFORMED_IMAGE_NAME}:${IMAGE_TAG}
