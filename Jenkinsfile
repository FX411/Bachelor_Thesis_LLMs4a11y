pipeline {
    ...
    environment {
        IMAGE_NAME = "openai_manual_reports-website"
        IMAGE_TAG = "latest"
        CONTAINER_NAME = "openai_manual_reports-website"
        NETWORK_NAME = "openai_manual_reports-network"
        REPORTS_DIR = "reports"
            ...
            steps {
                script {
                    sh '''
                        ...
                        docker run -d --network=${NETWORK_NAME} -p 4001:4001 --name ${CONTAINER_NAME} ${IMAGE_NAME}:${IMAGE_TAG}
            ...
            steps {
                script {
                    sh '''
                        ...
                        docker build -t pa11y-ci-tester-openai_manual_reports -f Dockerfile.pa11y .

                        echo "Erstelle Reports-Ordner..."
                        mkdir -p ${REPORTS_DIR}

                        echo "Starte Accessibility-Tests..."
                        docker run --rm --network=${NETWORK_NAME} -v $PWD/${REPORTS_DIR}:/app/reports pa11y-ci-tester-openai_manual_reports || true
